...
import { InventoryReservationDto } from "../dtos/all-dto";
...
@ApiTags("InventoryReservation Query")
@UseGuards(InventoryReservationAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("inventoryreservations/query")
export class InventoryReservationQueryController {
  #logger = new Logger(InventoryReservationQueryController.name);

  constructor(private readonly service: InventoryReservationQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all inventoryreservation with optional pagination" })
  @ApiResponse({ status: 200, type: InventoryReservationsResponse })
  @ApiQuery({ name: "options", required: false, type: InventoryReservationDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<InventoryReservation>    
  ): Promise<InventoryReservationsResponse<InventoryReservation>> {
    try {
     
      const inventoryreservations = await this.service.findAll(options);
      logger.info("Retrieving all inventoryreservation");
  ...
  @ApiOperation({ summary: "Find inventoryreservation by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter inventoryreservation', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: InventoryReservationsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "InventoryReservation no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find inventoryreservations with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<InventoryReservation>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all inventoryreservations" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count inventoryreservations with conditions" })
  @ApiResponse({ status: 200, type: InventoryReservationsResponse<InventoryReservation> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades InventoryReservations no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one inventoryreservation with conditions" })
  ...
  @ApiOperation({ summary: "Find one inventoryreservation or return error" })
  @ApiResponse({ status: 200, type: InventoryReservationResponse<InventoryReservation> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad InventoryReservation no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get inventoryreservation by ID" })
  @ApiResponse({ status: 200, type: InventoryReservationResponse<InventoryReservation> })
  @ApiResponse({ status: 404, description: "InventoryReservation not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the inventoryreservation to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<InventoryReservationResponse<InventoryReservation>> {
    try {
      const inventoryreservation = await this.service.findOne({ where: { id } });
      if (!inventoryreservation) {
        throw new NotFoundException(
          "InventoryReservation no encontrado para el id solicitado"
        );
      }
