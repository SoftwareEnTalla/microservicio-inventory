...
import { InventoryDto } from "../dtos/all-dto";
...
@ApiTags("Inventory Query")
@UseGuards(InventoryAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("inventorys/query")
export class InventoryQueryController {
  #logger = new Logger(InventoryQueryController.name);

  constructor(private readonly service: InventoryQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all inventory with optional pagination" })
  @ApiResponse({ status: 200, type: InventorysResponse })
  @ApiQuery({ name: "options", required: false, type: InventoryDto }) // Ajustar según el tipo real
  ...
  @LogExecutionTime({
    ...
    client: LoggerClient.getInstance()
      .registerClient(InventoryQueryService.name)
      .get(InventoryQueryService.name),
  })
  async findAll(
    @Query("options") options?: FindManyOptions<Inventory>    
  ): Promise<InventorysResponse<Inventory>> {
    try {
     
      const inventorys = await this.service.findAll(options);
  ...
  @ApiOperation({ summary: "Find inventory by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter inventory', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: InventorysResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Inventory no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find inventorys with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Inventory>,
    ...
    try {
     const paginationArgs: PaginationArgs = PaginationArgs.createPaginator(
      ...
      if (!entities) {
        throw new NotFoundException("Entidades Inventorys no encontradas.");
      }
      return entities;
    } catch (error) {
      logger.error(error);
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all inventorys" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count inventorys with conditions" })
  @ApiResponse({ status: 200, type: InventorysResponse<Inventory> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Inventorys no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one inventory with conditions" })
  ...
  async findOne(
    ...
    try {
      ...
      if (!entity) {
        throw new NotFoundException("Entidad Inventory no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find one inventory or return error" })
  @ApiResponse({ status: 200, type: InventoryResponse<Inventory> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Inventory no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the inventory to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<InventoryResponse<Inventory>> {
    try {
      const inventory = await this.service.findOne({ where: { id } });
      if (!inventory) {
        throw new NotFoundException(
          "Inventory no encontrado para el id solicitado"
        );
      }
      return inventory;
    } catch (error) {
